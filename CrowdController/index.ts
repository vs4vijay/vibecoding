...
Please proceed with the implementation now.`;
    ...
    // Execute OpenCode CLI with real-time output
    console.log("\n🤖 Executing OpenCode CLI (streaming output)...");
    console.log(`⏱️  Timeout: ${config.timeoutMinutes} minutes`);
    console.log("─".repeat(60));

    const opencodeModel = process.env.OPENCODE_MODEL || "anthropic/claude-sonnet-4-20250514";
    ...
    const exitCode = await opencodeProcess.exited;
    clearTimeout(timeout);
    console.log("─".repeat(60));

    if (exitCode !== 0) {
      throw new Error(`OpenCode CLI failed with exit code ${exitCode}`);
    }
    ...
    if (!hasChanges) {
      console.log("\nℹ️  No changes made by OpenCode");
      console.log("💡 This might mean:");
      console.log("   - Files were created outside the git repository");
      console.log("   - OpenCode didn't actually write files");
...
ENVIRONMENT VARIABLES:
  ...
  OPENCODE_MODEL          OpenCode model in provider/model format (default: anthropic/claude-sonnet-4-20250514)
  ANTHROPIC_API_KEY       Required: Anthropic API key (or other provider key)


SETUP:
  1. Install Bun: https://bun.sh
  2. Install OpenCode CLI: curl -fsSL https://opencode.ai/install | bash
  3. Set ANTHROPIC_API_KEY (or other provider key) environment variable
