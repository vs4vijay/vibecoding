# Tiny Fighters Completion Plan — Vertical Slices
...
**Spec:** `docs/superpowers/specs/2026-08-24-tiny-fighters-design.md` (+ `sections/01–04`).
**Predecessor:** `docs/superpowers/plans/2026-08-24-tiny-fighters-mvp.md` — its Tasks 1–11 are **committed** (HEAD `0464519`). Its Task 12 (UI shell) exists as **uncommitted WIP** on the working tree. This plan supersedes Tasks 12–15; do not re-read them as pending work.
...
export function createTitleScene(humanSlots: number[] = [0]): Scene {
  ...
  const logo = el("div", { cls: "logo", text: "TINY FIGHTERS" });
...
Title → Controls → REMAP (persisted under `tiny.bindings.v1`).
