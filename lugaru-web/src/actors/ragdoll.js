...
function capsuleVolume(halfH, r) {
    return Math.PI * r * r * (2 * halfH + (4 / 3) * r);
}
function buildJointTable(boneLen) {
...
export function spawnRagdoll(world, rig, impulse, opts = {}) {
    ...
    const groundAt = (x, z) => world.castGround(x, z);
    let lift = 0;
    ...
    for (const spec of buildJointTable(rig.boneLen)) {
        ...
        if (!pb || !cb)
            continue;
        world.raw.createImpulseJoint(RAPIER.JointData.spherical(spec.anchor, { x: 0, y: 0, z: 0 }), pb, cb, true);
    }
    // ---- apply impulse ----
    ...
    spineBody.applyImpulseAtPoint({ x: ndx * extraDv * spineMass, y: ndy * extraDv * spineMass, z: ndz * extraDv * spineMass }, { x: spineWPos.x, y: spineWPos.y, z: spineWPos.z }, true);
