...
import { BASE_POSE, CLIPS, type ClipData, type PoseTable } from './clipsData';

/** Full pose: absolute euler per bone. */
export type Pose = Record<BoneName, [number, number, number]>;

export interface Clip {
  durMs: number;
  loop: boolean;
  keys: { t: number; pose: PoseTable }[];
}
 ...
 */
export function samplePose(clip: Clip, tMs: number, out: Pose): void {
...
function resolveInto(pose: PoseTable, out: Pose): void {
  for (let b = 0; b < BONE_NAMES.length; b++) {
...
interface Layer {
  clip: ClipData;
  /** Global player time at which this layer started. */
  startMs: number;
  /** Ramp-in duration; weight = min(1, local / fadeInMs). */
  fadeInMs: number;
}
...
export class ClipPlayer {
  ...
  private readonly accPose: Pose;
  private readonly tmpPose: Pose;
  ...
  play(name: string, fadeMs?: number): void {
  ...
  private pushLayer(clip: ClipData, fadeInMs: number): void {
    this.layers.unshift({ clip, startMs: this.nowMs, fadeInMs });
    if (this.layers.length > MAX_LAYERS) this.layers.length = MAX_LAYERS;
  }
}
