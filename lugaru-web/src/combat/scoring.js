 ...
 */
import { COMBO_CHAIN_POINTS, COMBO_DECAY_NUMERATOR, COMBO_WINDOW_MS, SCORE_LEG_CANNON, SCORE_NICE_AIM, SCORE_NINJA_THROW, SCORE_REVERSAL, SCORE_REVERSAL_KO, SCORE_STEALTH_KILL, SCORE_STYLE_WALLKICK, } from '../data/tuning';
...
export class ScoreLedger {
    ...
    award(event) {
        ...
        if (points === undefined) {
            throw new Error(`unknown score event: ${String(event.type)}`);
        }
        this.score += points;
 ...
 */
export function registerComboHit(ledger, nowMs) {
