...
/**
 * One fighter mid-swing. An alias, not a wrapper type: every field the clash
 * rules need already lives on the fighter snapshot (phase, currentMove,
 * weapon, durability, pos, heading) and applyHit established the convention
 * of plain states crossing module borders.
 */
export type ArmedSwing = FighterState;

/** Why a weapon left its wielder's hand. */
export type DropReason = 'clash' | 'disarm' | 'ko';

/**
 * A weapon came loose. game.ts spawns the physical box:
 * WeaponDrops.spawnDrop(weaponClass, pos, vel).
 */
export interface DropEvent {
  type: 'drop';
  reason: DropReason;
  weaponClass: WeaponId;
  /** Fighter that lost the weapon. */
  fromId: string;
  /** Spawn position of the Rapier body (the wielder's position at hand-off). */
  pos: Vec3;
  /** Initial velocity of the Rapier body (m/s). */
  vel: Vec3;
}

/** onReversalVsArmed's payload: a drop with the disarm reason pinned. */
export type DisarmEvent = DropEvent & { reason: 'disarm' };

/**
 * A knife left a hand in flight. game.ts launches the Rapier body:
 * Projectiles.throwKnife(from, dir, speed).
 */
export interface ThrowKnifeEvent {
  type: 'throw';
  weaponClass: 'knife';
  fromId: string;
  /** Chest-height launch position. */
  from: Vec3;
  /** Unit aim direction, ground plane. */
  dir: { x: number; z: number };
  /** Launch speed (m/s). */
  speed: number;
}

/** Everything the sim can hand the game layer in one step. */
export type WeaponSimEvent = DropEvent | ThrowKnifeEvent;

/** Outcome of a successful clash. */
export interface ClashResult {
  /** Weapons knocked loose this clash — bridge each via WeaponDrops. */
  drops: DropEvent[];
}

/** Analytic result of a thrown knife connecting. */
export interface ThrownKnifeOutcome {
  damageDealt: number;
  fatal: boolean;
  /** The fighter the blade is lodged in. */
  stuckIn: string;
  /** Who threw it (provenance for retrieval/attribution). */
  thrownBy: string;
}
...
export function heldWeapon(f: FighterState): WeaponId | null {
 ...
 */
function isArmedSwing(f: FighterState): boolean {
  if (f.phase.t !== 'active' || heldWeapon(f) === null) return false;
...
function facesSwing(watcher: FighterState, other: FighterState, halfArc: number): boolean {
  const dx = other.pos.x - watcher.pos.x;
  const dz = other.pos.z - watcher.pos.z;
  if (dx === 0 && dz === 0) return true; // co-located: trivially facing
 ...
 */
export function weaponDropEvent(
  f: FighterState,
  awayFrom: FighterState | null,
  reason: DropReason,
): DropEvent {
 ...
 */
export function tryClash(a: ArmedSwing, b: ArmedSwing, rng: () => number): ClashResult | null {
  if (!isArmedSwing(a) || !isArmedSwing(b)) return null;
  ...
  if (dx * dx + dz * dz > maxReach * maxReach) return null;

  // …and each swing must be aimed at the other (shared slash arc).
  const halfArc = MOVES.slash.arcRad / 2 + 1e-9;
  if (!facesSwing(a, b, halfArc) || !facesSwing(b, a, halfArc)) return null;
 ...
 */
export function onReversalVsArmed(
  reverser: FighterState,
  armedVictim: FighterState,
): DisarmEvent | null {
  if (heldWeapon(armedVictim) === null) return null;
  const event = weaponDropEvent(armedVictim, reverser, 'disarm');
  stripWeapon(armedVictim, false);
  return event as DisarmEvent;
}
 ...
 */
export function throwKnife(state: FighterState, dir: { x: number; z: number }): ThrowKnifeEvent | null {
  if (heldWeapon(state) === null || !WEAPONS[heldWeapon(state)!].throwable) return null;
  const len = Math.hypot(dir.x, dir.z);
  if (len < 1e-9) return null; // no aim, no throw
  const event: ThrowKnifeEvent = {
    type: 'throw',
    weaponClass: 'knife',
    fromId: state.id,
    from: {
 ...
 */
export function thrownKnifeHit(victim: FighterState, thrower: FighterState): ThrownKnifeOutcome {
 ...
 */
export function cleanBlade(f: FighterState): boolean {
  if (f.bloodiedWeapon !== true) return false;
