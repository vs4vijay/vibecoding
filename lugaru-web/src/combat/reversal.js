...
import { COUNTER_WINDOW_MS, REVERSE_DAMAGE, REVERSAL_HALF_ANGLE_RAD, } from '../data/tuning';
/**
 ...
 */
export function tryReversal(defender, incoming) {
    const win = incoming.def.reversalWindow;
    if (win === undefined)
        return 'late';
    if (incoming.elapsedMs < win.from)
        return 'early';
    if (incoming.elapsedMs > win.to)
        return 'late';
    const dx = incoming.attacker.pos.x - defender.pos.x;
    const dz = incoming.attacker.pos.z - defender.pos.z;
    if (dx === 0 && dz === 0)
        return 'success'; // co-located: trivially facing
    const bearing = Math.atan2(-dx, -dz); // heading that would face the attacker
    if (angleDiff(defender.heading, bearing) >= REVERSAL_HALF_ANGLE_RAD)
        return 'notFacing';
    return 'success';
}
/**
 ...
 */
export function startCounter(originalAttacker, reverserId) {
    if (originalAttacker.phase.t !== 'hitstun' ||
