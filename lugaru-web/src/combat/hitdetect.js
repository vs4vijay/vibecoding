...
export function forwardXZ(heading) {
 ...
 */
export function findHit(attacker, victims) {
    if (attacker.phase.t !== 'active')
        return [];
    ...
    for (let i = 0; i < victims.length; i++) {
        ...
        if (Math.abs(Math.atan2(cross, dot)) > halfArc)
        ...
        events.push({
            ...
            victimId: v.id,
            moveId,
            dirVector: invDist > 0 ? { x: dx * invDist, z: dz * invDist } : { x: fdx, z: fdz },
        });
    }
    return events;
}
 ...
 */
export function applyHit(hit, fighters) {
    let attacker;
    let victim;
    for (let i = 0; i < fighters.length && (attacker === undefined || victim === undefined); i++) {
        const f = fighters[i];
        if (f.id === hit.attackerId)
            attacker = f;
        else if (f.id === hit.victimId)
            victim = f;
    }
 ...
 */
export function applyLethalState(victim) {
    victim.hp = 0;
 ...
 */
export function downFighter(victim) {
    victim.currentMove = undefined; // getting hit interrupts whatever ran
    victim.moveElapsedMs = 0;
    victim.phase.moveId = undefined;
