...
/** One injury-model transition, plain data for FX/score hooks. */
export interface InjuryEvent {
  type: 'bled' | 'limped' | 'knockedOut';
}
 ...
 */
const NO_EVENTS: InjuryEvent[] = [];
const BLED_ONLY: InjuryEvent[] = [{ type: 'bled' }];
const LIMPED: InjuryEvent = { type: 'limped' };
const KNOCKED_OUT: InjuryEvent = { type: 'knockedOut' };
 ...
 */
export function updateInjuries(f: FighterState, dtMs: number): InjuryEvent[] {
  // Already out — terminal until Task 14's ragdoll/respawn layer.
  if (f.flags.unconscious) return NO_EVENTS;

  if (f.flags.bleeding && f.hp > BLEED_HP_FLOOR) {
    f.hp -= (dtMs / 1000) * BLEED_DPS;
    if (f.hp < BLEED_HP_FLOOR) f.hp = BLEED_HP_FLOOR;
    return BLED_ONLY;
  }
...
function knockOut(f: FighterState): void {
