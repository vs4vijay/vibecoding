...
import { BODY_THROW_IMPACT_DAMAGE, BODY_THROW_SPEED_MPS, FLIP_STUN_MS, KNOCKDOWN_VELY, LEG_CANNON_KNOCK_SPEED_MPS, WALL_KICK_LAUNCH_MPS, } from '../data/tuning';
 ...
 */
export function applySpecial(move, attacker, target, ctx) {
    ...
    switch (move) {
        ...
        // ------------------------------------------------------------------
        case 'tackle': {
            if (target === null || distXZ(attacker, target) > def.rangeM)
                return null;
            return {
                damage: def.damage,
                knockdown: true,
        ...
        // ------------------------------------------------------------------
        case 'soccerKick': {
            if (target === null || !isCorpse(target))
                return null;
            if (distXZ(attacker, target) > def.rangeM)
                return null;
        ...
        case 'airGrab': {
            if (target === null)
                return null;
            if (target.stance !== 'airborne')
                return null;
            if (target.flags.invulnerableAirFlipMs > 0)
                return null;
            if (distXZ(attacker, target) > def.rangeM)
                return null;
            return {
        ...
        case 'bodyThrow': {
            if (ctx.corpseImpact === true) {
                if (target === null || isCorpse(target))
                    return null; // corpses hit the living
        ...
        case 'flip': {
            ...
            if (distXZ(attacker, target) > def.rangeM)
                return null;
            if (target.phase.t === 'ko' || isCorpse(target))
                return null;
