...
import { ACCEL, BODY_THROW_SPEED_MPS, COUNTER_WINDOW_MS, FLIP_STUN_MS, FRICTION, INPUT_BUFFER_MS, GRAVITY, JUMP_SPEED, RECOVERY_CHAIN_MIN_MS, REVERSE_ATTEMPT_MS, RUN_STANCE_SPEED, TARGET_SENSE_RADIUS_M, TURN_RATE, } from '../data/tuning';
...
import { heldWeapon, onReversalVsArmed, thrownKnifeHit, weaponDropEvent, } from './weaponsLogic';
import { applySpecial } from './bodymoves';
/** Shared quiet-step value for lastCombatEvents — see the field's doc. */
const NO_COMBAT_EVENTS = [];
/** Shared quiet-step value for lastInjuryEvents — see the field's doc. */
const NO_INJURY_EVENTS = [];
...
export class FighterSim {
    ...
    get horizontalVelX() { return this.locoVelX; }
    get horizontalVelZ() { return this.locoVelZ; }
     ...
     */
    lastInjuryEvents = NO_INJURY_EVENTS;
    /**
     ...
     * allocation happens.
     */
    lastCombatEvents = NO_COMBAT_EVENTS;
    ...
    constructor(species, id, isPlayer) {
    ...
    collectHits(victims) {
        ...
        for (let i = 0; i < events.length; i++) {
            const e = events[i];
            if (this.swingHitSet.has(e.victimId))
                continue;
     ...
     */
    applySpecialStrike(hit, fighters) {
        if (hit.attackerId !== this.state.id)
            return null; // awards land on the attacker's sim
        let victim;
        for (let i = 0; i < fighters.length && victim === undefined; i++) {
            if (fighters[i].id === hit.victimId)
                victim = fighters[i];
        }
        if (victim === undefined)
            return null;
        ...
        if (effect === null)
            return null; // gate-rejected: a whiff, not a hit
     ...
     */
    applyCorpseImpact(victimId, fighters) {
        let victim;
        for (let i = 0; i < fighters.length && victim === undefined; i++) {
            if (fighters[i].id === victimId)
                victim = fighters[i];
        }
        if (victim === undefined)
            return null;
        const ctx = { ...this.specialCtx(), corpseImpact: true };
     ...
     */
    applyThrownKnifeImpact(victim, thrower) {
        if (thrower.id !== this.state.id)
            return null;
        const outcome = thrownKnifeHit(victim, thrower);
        if (outcome.fatal)
            this.ledger?.award({ type: 'NINJA_THROW' });
        return outcome;
    }
    /**
     ...
     */
    applyEffect(effect, attacker, victim, dirVector) {
        if (effect.damage !== undefined && effect.damage > 0) {
            ...
            if (victim.hp <= 0) {
                ...
                if (effect.killScoreEvent !== undefined)
                    this.ledger?.award(effect.killScoreEvent);
            }
            ...
            if (effect.scoreEvent !== undefined)
                this.ledger?.award(effect.scoreEvent);
        }
        else if (effect.stunMs !== undefined) {
    ...
    fireFlip() {
        ...
        for (let i = 0; i < fighters.length; i++) {
            ...
            if (effect?.stunMs !== undefined)
                this.applyStun(other, effect.stunMs);
        }
    }
    ...
    consumeAttack(input, phaseAtPress) {
        const button = input.pressed.attack ? 'attack' : 'jump';
        const action = resolveAction({ button, heldAttack: input.held.attack }, this.snapshot(), this.worldCtx());
     ...
     */
    dispatchMoveAction(id, phaseAtPress) {
     ...
     */
    consumeCrouch(input, phaseAtPress) {
        const action = resolveAction({ button: 'crouch' }, this.snapshot(), this.worldCtx());
        if (action === null)
            return; // plain duck: crouch stance via integrate()
     ...
     */
    executeReversal(attackerId) {
        const s = this.state;
        const fighters = this.world.fighters;
        let attacker;
        for (let i = 0; i < fighters.length && attacker === undefined; i++) {
            if (fighters[i].id === attackerId)
                attacker = fighters[i];
        }
        ...
        if (outcome !== 'success')
            return; // early/late/notFacing → whiffed duck
    ...
    consumeCounter() {
        ...
        if (reverser.hp <= 0)
            this.ledger?.award({ type: 'REVERSAL_KO' });
    ...
    applyBuffer(dt) {
        ...
        if (this.state.phase.t !== 'idle') {
            if (this.bufferMsLeft <= 0)
                this.bufferedActionId = null; // expired waiting
            return;
        }
        if (this.bufferMsLeft > 0)
            this.startMove(id); // still alive → fire
        else
            this.bufferedActionId = null; // window spent unspent
    }
    // -- physics --------------------------------------------------------------
    /** Integrate locomotion + gravity + ground; stance follows phase/motion. */
    integrate(dt, moveX, moveZ) {
        const s = this.state;
        const dtS = dt / 1000;
    ...
    snapshot() {
        ...
        return {
            ...
            currentMove: movePhase !== undefined
 ...
 */
function nearestTargetOf(self, world) {
    ...
    return {
        ...
        incomingAttack: best.phase.t === 'startup' || best.phase.t === 'active'
            ? {
                attackerId: best.id,
...
function facesToward(watcher, other) {
