...
import { LEG_CANNON_AHEAD_HALF_RAD, REVERSE_PRESS_WINDOW_MS, STEALTH_BEHIND_HALF_RAD, } from '../data/tuning';
export function resolveAction(input, actor, worldCtx) {
    switch (input.button) {
        case 'attack':
...
// ---------------------------------------------------------------------------
function resolveAttack(input, a) {
    ...
    if (t?.isDowned && t.dist <= MOVES.soccerKick.rangeM)
        return { kind: 'move', id: 'soccerKick' };
    if (t?.airborne && t.dist <= MOVES.airGrab.rangeM)
        return { kind: 'move', id: 'airGrab' };
    ...
    if (a.stance === 'crouched')
        return { kind: 'move', id: 'legSweep' };
    if (a.isRunning || a.stance === 'running')
        return { kind: 'move', id: 'runningKick' };
    ...
    if (!t || t.dist > MOVES.punch.rangeM)
        return null;
...
// ---------------------------------------------------------------------------
function resolveJump(a, ctx) {
    if (ctx.airborneSelf || a.stance === 'airborne')
        return { kind: 'move', id: 'flip' };
    const t = a.nearestTarget;
    if ((a.isRunning || a.stance === 'running') &&
        t &&
...
function resolveCrouch(a, ctx) {
    ...
    if (a.stance === 'crouched') {
        ...
        if (ctx.weaponOnGroundNearby)
            return { kind: 'move', id: 'pickupOrContext' };
        if (ctx.downedBodyNearby)
            return { kind: 'move', id: 'bodyThrow' };
        if (a.hasWeapon && a.bladeBloody)
            return { kind: 'move', id: 'cleanBlade' };
        // Crouched with nothing to do stays put.
        return null;
    }
    if (a.isRunning || a.stance === 'running')
        return { kind: 'move', id: 'slideStop' };
    return null;
}
/**
...
function findReversal(a) {
    ...
    if (!win)
        return null;
    const elapsed = inc.phase === 'startup'
        ? inc.phaseMsElapsed
        : inc.phase === 'active'
