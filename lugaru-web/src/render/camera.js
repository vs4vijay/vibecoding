...
export class ChaseCamera {
    ...
    update(dtSec, targetPos, targetHeading, lookDX, lookDY) {
        this.yaw -= lookDX * 0.0022;
        this.pitch += lookDY * 0.0022;
        if (this.pitch < PITCH_MIN)
            this.pitch = PITCH_MIN;
        else if (this.pitch > PITCH_MAX)
            this.pitch = PITCH_MAX;
        // Spherical offset around the target head point.
        const headY = targetPos.y + HEAD_HEIGHT;
        const cosPitch = Math.cos(this.pitch);
        this.offset.set(Math.sin(this.yaw) * cosPitch, Math.sin(this.pitch), Math.cos(this.yaw) * cosPitch).multiplyScalar(this.distance);
        this.desired.set(targetPos.x + this.offset.x, headY + this.offset.y, targetPos.z + this.offset.z);
