...
export class FxParticles {
    ...
    positions;
    velocities;
    lifes = new Float32Array(BLOOD_PUFF_SLOTS);
    nextPuff = 0;
    // Dust rings: small pool of expanding, fading flat ring meshes.
    rings = [];
    ringLifes = new Float32Array(DUST_RING_SLOTS);
    nextRing = 0;
    ...
    constructor(scene) {
        ...
        for (let i = 0; i < DUST_RING_SLOTS; i++) {
            const ring = new THREE.Mesh(new THREE.RingGeometry(0.42, 0.55, 24), new THREE.MeshBasicMaterial({
                color: DUST_COLOR,
                transparent: true,
                opacity: 0,
                depthWrite: false,
