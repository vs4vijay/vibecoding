...
import { spawnRagdoll } from './actors/ragdoll';
/**
...
export class Game {
    ...
    simStep(dtMs) {
        ...
        for (const hit of pHits)
            this.landHit(hit, this.playerSim);
        ...
        for (const hit of dHits)
            this.landHit(hit, this.dummySim);
    ...
    updateRender(dtMs) {
        ...
        this.chaseCam.update(dtMs / 1000, this.playerSim.state.pos, this.playerSim.state.heading, this.frame.lookDX, this.frame.lookDY);
    ...
    applyPhaseClip(sim, ctrl) {
        ...
        switch (phase) {
            ...
            case 'recovery':
                if (moveId)
                    ctrl.setPhaseOverride(moveClipKey(moveId));
                else
