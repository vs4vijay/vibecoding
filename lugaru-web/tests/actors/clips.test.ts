...
import { samplePose, ClipPlayer, type Pose } from '../../src/actors/clips';
import { BASE_POSE, CLIPS } from '../../src/actors/clipsData';
import { buildRig, type BoneName } from '../../src/actors/skeleton';
...
describe('ClipPlayer', () => {
  ...
  it('crossfades without snapping: pose starts at old clip, ends at new', () => {
    ...
    for (let i = 0; i < 20; i++) p.update(16.7); // > fade window
    p.applyTo(rig);
  ...
  it('retires the underlying layer once the fade completes', () => {
    ...
    p.update(300);
    p.play('crouchWalk');
    for (let i = 0; i < 30; i++) p.update(16.7);
...
describe('clip data integrity', () => {
  ...
  it('looping clips end where they start (wrap seam)', () => {
    for (const [name, clip] of Object.entries(CLIPS)) {
      ...
      for (const b of Object.keys(first) as BoneName[]) {
