...
describe('CharacterController kinematics', () => {
  ...
  it('reaches run speed under sustained forward input (accel-limited)', () => {
    ...
    const f = makeFrame();
    f.moveZ = -1;
    for (let i = 0; i < 60; i++) c.update(DT, f, false, 0); // 1s
  ...
  it('stops via friction when input releases', () => {
    ...
    const f = makeFrame();
    f.moveZ = -1;
    for (let i = 0; i < 60; i++) c.update(DT, f, false, 0);
    f.moveZ = 0;
    for (let i = 0; i < 120; i++) c.update(DT, f, false, 0); // 2s
  ...
  it('jump leaves the ground ballistically and lands again', () => {
    ...
    for (let i = 0; i < 10; i++) c.update(DT, f, false, 0);
    const gy0 = c.pos.y;
  ...
  it('crouch accumulates crouchHeldMs while held, resets on release', () => {
    ...
    const f = makeFrame();
    f.held.crouch = true;
    for (let i = 0; i < 30; i++) c.update(DT, f, false, 0);
  ...
  it('locked input freezes intent but physics settles', () => {
    ...
    open.moveZ = -1;
    for (let i = 0; i < 30; i++) c.update(DT, open, false, 0);
    ...
    for (let i = 0; i < 30; i++) c.update(DT, locked, true, 0);
  ...
  it('heading turns toward velocity direction', () => {
    ...
    for (let i = 0; i < 90; i++) c.update(DT, f, false, 0);
  ...
  it('aligns pitch to uphill slope when walking up', () => {
    ...
    c.pos.x = 15;
    c.pos.z = 0;
    for (let i = 0; i < 240; i++) c.update(DT, f, false, 0); // 4s of running
    if (!c.grounded) return; // skip if launch happened — shouldn't
...
describe('clip selection', () => {
  it('airborne picks jump then fall', () => {
    ...
    for (let i = 0; i < 40; i++) c.update(DT, f, false, 0);
    // After apex, still airborne → fall clip took over.
    if (!c.grounded) expect(c.anim.current).toBe('fall');
    for (let i = 0; i < 60 && !c.grounded; i++) c.update(DT, f, false, 0);
    expect(c.grounded).toBe(true);
  });
  ...
  it('running stance drives the run clip', () => {
    ...
    const f = makeFrame();
    f.moveZ = -1;
    for (let i = 0; i < 45; i++) c.update(DT, f, false, 0);
    expect(c.stance).toBe('running');
