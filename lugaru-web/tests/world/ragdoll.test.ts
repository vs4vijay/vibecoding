...
describe('Ragdoll', () => {
  it('all bones fall below initial chest height after 60 steps', () => {
    ...
    for (const bone of Object.values(rig.bones)) bone.updateMatrixWorld(true);
    ...
    for (const [name, pose] of Object.entries(handle.bones)) {
      expect(
        pose.pos.y,
        `bone ${name} should have fallen`,
      ).toBeLessThan(initialChestY + 0.5);
    }
  });
...
describe('Projectiles', () => {
  it('knife thrown at flat ground lands and reports rest position within bounds', () => {
    ...
    let restEvent: { type: 'rest'; at: { x: number; y: number; z: number } } | null = null;
