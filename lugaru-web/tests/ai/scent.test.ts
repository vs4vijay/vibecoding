...
function emittersAt(
  x: number,
  z: number,
  rate: number = SCENT_EMIT_RATE_BASE,
): ScentEmitter[] {
  return [{ pos: { x, z }, rate }];
}
...
describe('ScentField.intensityAt', () => {
  ...
  it('accumulates intensity at the emitter location', () => {
    ...
    const dt = 16.7;
    const steps = 10;
    for (let i = 0; i < steps; i++) f.update(dt, emittersAt(0, 0), { x: 0, z: 0 });
    expect(f.intensityAt({ x: 0, z: 0 })).toBeGreaterThan(0);
  });

  it('is clamped to the arena bounds when sampling', () => {
    const f = new ScentField();
    for (let i = 0; i < 4; i++) f.update(16.7, emittersAt(0, 0), { x: 0, z: 0 });
...
describe('WindSystem', () => {
  ...
  it('random-walks after each WIND_TURN_MS step (direction drifts ≤ ±30°)', () => {
    ...
    while (delta > Math.PI) delta = Math.abs(delta - 2 * Math.PI); // wrap
