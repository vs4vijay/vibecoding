...
import type { MoveDef, MoveId } from '../../src/combat/types';
...
function makeIncoming(moveId: MoveId, elapsedMs: number) {
...
describe('tryReversal', () => {
  ...
  it('moves without a reversalWindow are never reversible → late', () => {
    ...
    expect(
      tryReversal(defender, { attacker, def: MOVES.soccerKick, elapsedMs: 10 }),
    ).toBe('late');
  });
});
...
// ---------------------------------------------------------------------------
// Integration — real FighterSim.update steps, no clock/RNG anywhere.
// ---------------------------------------------------------------------------
...
function clickCrouch(): InputFrame {
...
function runUntil(
  ...
  for (let i = 0; i < maxSteps && !pred(); i++) sim.update(STEP_MS, input, world);
  return pred();
}
