...
describe('attack button', () => {
    ...
    it('held attack during first punch recovery chains doublePunch (crouch not held)', () => {
        const a = resolveAction({ button: 'attack', heldAttack: true }, makeActor({ currentMove: { id: 'punch', phase: 'recovery', phaseMsElapsed: 10 } }), NO_CTX);
        expectMove(a, 'doublePunch');
    });
    it('doublePunch does NOT chain when crouch is held (reverse intent wins)', () => {
        const a = resolveAction({ button: 'attack', heldAttack: true }, makeActor({
            stance: 'crouched',
            crouchHeldMs: 100,
    ...
    it('enemy downed in front → soccerKick', () => {
        expectMove(resolveAction({ button: 'attack' }, makeActor({ nearestTarget: makeTarget({ dist: 2.2, isDowned: true }) }), NO_CTX), 'soccerKick');
    });
    it('enemy airborne overhead → airGrab', () => {
        expectMove(resolveAction({ button: 'attack' }, makeActor({ nearestTarget: makeTarget({ dist: 1.8, airborne: true }) }), NO_CTX), 'airGrab');
    });
    it('wall within 0.9m → wallKick', () => {
        expectMove(resolveAction({ button: 'attack' }, makeActor({ wallProximityM: 0.7 }), NO_CTX), 'wallKick');
    });
    it('wall beyond 0.9m falls through to normal attack resolution', () => {
        const a = resolveAction({ button: 'attack' }, makeActor({ wallProximityM: 1.4, nearestTarget: makeTarget() }), NO_CTX);
        expectMove(a, 'punch');
    });
    it('behind-unaware target beats downed/airborne/wall context moves', () => {
        ...
        const a = resolveAction({ button: 'attack' }, makeActor({
            wallProximityM: 0.5,
    ...
    it('unaware target NOT behind → normal punch (stealth needs behind)', () => {
        const a = resolveAction({ button: 'attack' }, makeActor({ nearestTarget: makeTarget({ unaware: true }) }), NO_CTX);
        expectMove(a, 'punch');
    });
    it('out-of-range standing target → nothing (punch range gate)', () => {
        const a = resolveAction({ button: 'attack' }, makeActor({ nearestTarget: makeTarget({ dist: 99 }) }), NO_CTX);
        expect(a).toBeNull();
    });
});
describe('jump button', () => {
    ...
    it('airborne + jump pressed → flip', () => {
        expectMove(resolveAction({ button: 'jump' }, makeActor({ stance: 'airborne' }), { ...NO_CTX, airborneSelf: true }), 'flip');
    });
    ...
    it('running near target → legCannon', () => {
        const a = resolveAction({ button: 'jump' }, makeActor({
            isRunning: true,
    ...
    it('running but target out of range → plain jump', () => {
        expectMove(resolveAction({ button: 'jump' }, makeActor({ isRunning: true, nearestTarget: makeTarget({ dist: 30 }) }), NO_CTX), 'jump');
    });
    it('running away from target → plain jump (needs target ahead)', () => {
        const a = resolveAction({ button: 'jump' }, makeActor({ isRunning: true, nearestTarget: makeTarget({ relAngle: Math.PI }) }), NO_CTX);
        expectMove(a, 'jump');
    });
});
describe('crouch button — reverse / context / slide-stop', () => {
    it('timed press vs incoming punch startup → reverse', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({
    ...
    it('crouched + press over ground weapon → pickupOrContext', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'crouched', crouchHeldMs: 200, hasWeapon: null }), { downedBodyNearby: false, weaponOnGroundNearby: true, airborneSelf: false });
        expectMove(a, 'pickupOrContext');
    });
    it('crouched + press over downed body → bodyThrow', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'crouched', crouchHeldMs: 200, hasWeapon: null }), { downedBodyNearby: true, weaponOnGroundNearby: false, airborneSelf: false });
        expectMove(a, 'bodyThrow');
    });
    it('weapon+body both nearby → weapon pickup wins', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'crouched', crouchHeldMs: 200, hasWeapon: null }), { downedBodyNearby: true, weaponOnGroundNearby: true, airborneSelf: false });
        expectMove(a, 'pickupOrContext');
    });
    it('holding bloody blade + press → cleanBlade (lowest context priority)', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'crouched', crouchHeldMs: 200, hasWeapon: 'knife', bladeBloody: true }), NO_CTX);
        expectMove(a, 'cleanBlade');
    });
    it('clean blade but no ground weapon underfoot → still cleans own blade', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'crouched', crouchHeldMs: 200, hasWeapon: 'sword', bladeBloody: true }), { downedBodyNearby: false, weaponOnGroundNearby: false, airborneSelf: false });
        expectMove(a, 'cleanBlade');
    });
    it('clean priority: pickup > body > clean (all contexts at once)', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'crouched', crouchHeldMs: 200, hasWeapon: 'knife', bladeBloody: true }), { downedBodyNearby: true, weaponOnGroundNearby: true, airborneSelf: false });
        expectMove(a, 'pickupOrContext');
    });
    it('crouch pressed while running → slideStop', () => {
        expectMove(resolveAction({ button: 'crouch' }, makeActor({ isRunning: true }), NO_CTX), 'slideStop');
    });
    it('crouched idle press with no context → nothing to do (null)', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'crouched', crouchHeldMs: 200 }), NO_CTX);
        expect(a).toBeNull();
    });
    ...
    it('airborne + crouch pressed → flip (second canonical trigger path)', () => {
        const a = resolveAction({ button: 'crouch' }, makeActor({ stance: 'airborne' }), { ...NO_CTX, airborneSelf: true });
        expectMove(a, 'flip');
    });
...
describe('resolver ordering and purity', () => {
    ...
    it('pure: same inputs → same output (sampled across truth table)', () => {
        for (let i = 0; i < 3; i++) {
            expect(resolveAction({ button: 'attack' }, makeActor({ nearestTarget: makeTarget() }), NO_CTX)).toEqual({ kind: 'move', id: 'punch' });
        }
    });
});
describe('MOVES table integrity', () => {
    ...
    it('non-combat moves carry no startup/active frames (context/loco only)', () => {
        ...
        for (const id of ['jump', 'hop', 'pickupOrContext', 'stealthKill', 'cleanBlade']) {
            const m = MOVES[id];
    ...
    it('windows stay inside startup+active for every reversal-capable move', () => {
        for (const id of ['punch', 'doublePunch', 'runningKick', 'legSweep']) {
            const m = MOVES[id];
