...
import type { InjuryEvent } from '../../src/combat/injury';
import type { FighterState } from '../../src/combat/stateMachine';
import { BLEED_DPS, LIMP_HP_FRACTION } from '../../src/data/tuning';
...
interface TickTally {
  ...
  limped: number;
  knockedOut: number;
  /** Number of updateInjuries calls the helper made. */
  steps: number;
}

/** Advance one fighter through `totalMs` of fixed `stepMs` steps, tallying events. */
function tick(f: FighterState, totalMs: number, stepMs = 1000 / 60): TickTally {
  const tally: TickTally = { bled: 0, limped: 0, knockedOut: 0, steps: 0 };
  for (let left = totalMs; left > 0; left -= stepMs) {
    const dt = Math.min(stepMs, left);
    for (const e of updateInjuries(f, dt)) tally[e.type as keyof TickTally]++;
    tally.steps++;
  }
  return tally;
}
...
function tickOnce(f: FighterState): InjuryEvent[] {
