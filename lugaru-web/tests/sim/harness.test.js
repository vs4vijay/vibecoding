...
import { makeFrame, forward, attackPress, attackHeld, repeatFrames, } from './scriptedInputs';
/**
 ...
 */
export function runScriptedFight(stepsMs, playerInputAt, aiInputs = () => null) {
...
describe('Headless scripted-fight harness (Task 10)', () => {
    it('spawns player rabbit + dummy wolf 3m apart; approach to within punch range, punch ×3; asserts ≥1 hit, enemy hp < max, no NaN, y ≥ heightAt, KO or timeout cleanly', () => {
        ...
        const summary = runScriptedFight(stepsMs, (i) => inputSequence[i] ?? makeFrame(), () => null // dummy AI = null input
        );
