...
describe("mute persistence", () => {
  test("toggleMute flips, persists lf2.muted, and reports the new state", () => {
    const s = fakeStorage();
    const audio = createAudio({ storage: s });
    expect(audio.toggleMute()).toBe(true);
    expect(s.store.get("lf2.muted")).toBe("true");
    expect(audio.toggleMute()).toBe(false);
    expect(s.store.get("lf2.muted")).toBe("false");
  });

  test("muted state survives reload (restored from storage)", () => {
    const s = fakeStorage();
    s.setItem("lf2.muted", "true");
    const audio = createAudio({ storage: s });
