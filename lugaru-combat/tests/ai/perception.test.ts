...
function sighter(over: Partial<Sighter> = {}): Sighter {
  return { pos: { x: 0, z: 0 }, heading: 0, species: 'wolf', ...over };
}

function target(over: Partial<SightTarget> = {}): SightTarget {
...
const EMPTY: { bushes: Bush[] } = { bushes: [] };
...
describe('canSee — range', () => {
  ...
  it('uses the rabbit\'s shorter sight range', () => {
    const rabbit: Sighter = { pos: { x: 0, z: 0 }, heading: 0, species: 'rabbit' };
...
describe('canSee — bush blocking', () => {
  const bushAt = (x: number, z: number): Bush => ({ pos: { x, z }, radius: BUSH_RADIUS_M });
...
describe('hear', () => {
  it('lets a rabbit hear a bush rustle at 10m but not a wolf (rabbit hears farther)', () => {
    const rs: Listener = { pos: { x: 0, z: 0 }, species: 'rabbit' };
    const wf: Listener = { pos: { x: 0, z: 0 }, species: 'wolf' };
    const e: HearingEvent = { kind: 'bushRustle', loudness: LOUDNESS_BUSH_RUSTLE, pos: { x: 0, z: -10 } };
    expect(hear(rs, e)).toBe(true);
    expect(hear(wf, e)).toBe(false);
  });

  it('records lastHeardPos on a heard event', () => {
    const l: Listener = { pos: { x: 0, z: 0 }, species: 'rabbit' };
    const e: HearingEvent = { kind: 'scream', loudness: 1.5, pos: { x: 3, z: -4 } };
    expect(hear(l, e)).toBe(true);
    expect(l.lastHeardPos).toEqual({ x: 3, z: -4 });
  });

  it('does not record lastHeardPos when out of range', () => {
    const l: Listener = { pos: { x: 0, z: 0 }, species: 'wolf' };
    const e: HearingEvent = { kind: 'bushRustle', loudness: LOUDNESS_BUSH_RUSTLE, pos: { x: 0, z: -100 } };
  ...
  it('hears bigger events (scream) from farther away', () => {
    const l: Listener = { pos: { x: 0, z: 0 }, species: 'rabbit' };
    const scream: HearingEvent = { kind: 'scream', loudness: 1.5, pos: { x: 0, z: -20 } };
    // 20m < rabbit scream radius (1.5 * 18 = 27).
    expect(hear(l, scream)).toBe(true);
    const rustle: HearingEvent = { kind: 'bushRustle', loudness: LOUDNESS_BUSH_RUSTLE, pos: { x: 0, z: -20 } };
