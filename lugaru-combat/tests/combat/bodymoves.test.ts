...
function makeEnemy(dist = 1.2, over: Partial<FighterState> = {}): FighterState {
  return makeFighter({
    id: 'wolf1',
    species: 'wolf',
    team: 1,
...
const OPEN_CTX: SpecialCtx = { wallProximityM: Infinity, crouchHeld: false };

interface Row {
  name: string;
  move: Parameters<typeof applySpecial>[0];
  attacker: FighterState;
  target: TargetRef | null;
  ctx: SpecialCtx;
  want: SpecialEffect | null;
}
...
// ---------------------------------------------------------------------------
// FighterSim wiring — the sim consumes SpecialEffects uniformly: damage,
// knockdown, stun, impulses, kill-attributed score awards.
// ---------------------------------------------------------------------------
...
describe('FighterSim applies special effects (T14)', () => {
  it('legCannon hit: 30 damage, knockdown, massive knockback push, LEG_CANNON award', () => {
    ...
    for (let i = 0; i < 60; i++) dummy.update(STEP_MS, null, world);
  ...
  it('mid-air flip stuns the nearby dummy for FLIP_STUN_MS and cancels grabs', () => {
    ...
    expect(
      applySpecial('airGrab', makeFighter(), makeEnemy(1.2, { stance: 'airborne', flags: { ...makeFighter().flags, invulnerableAirFlipMs: 100 } }), OPEN_CTX),
    ).toBeNull();
  });
  ...
  it('disarm tackle: crouch-held strike strips the victim’s weapon into a drop event', () => {
    ...
    const hits = strikeUntil(
      player,
      [dummy.state],
      world,
      makeInput({ held: { attack: false, jump: false, crouch: true } }),
    );
    expect(hits.length).toBe(1);
...
function strikeUntil(
  ...
  for (let i = 0; i < 200 && landed.length === 0; i++) {
    ...
    for (const hit of hits) {
      if (isSpecialMove(hit.moveId)) attacker.applySpecialStrike(hit, world.fighters);
      else applyHit(hit, world.fighters);
    }
    if (hits.length > 0) landed = hits;
  }
  return landed;
}
