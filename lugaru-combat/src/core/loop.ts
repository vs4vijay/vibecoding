...
export class FixedLoop {
  ...
  constructor(
    ...
    private readonly update: (stepDtMs: number) => void,
  ) {}
  ...
  advance(realDtMs: number): number {
    ...
    if (this.acc > this.stepMs * MAX_CATCH_UP) this.acc = 0; // drop debt beyond clamp window
    for (let i = 0; i < n; i++) this.update(this.stepMs);
    return n;
  }
}
