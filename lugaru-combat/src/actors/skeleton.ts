...
/** The eleven named bones of the quadruped rig. */
...
export interface Rig {
  /** Sim-positioned group: controller sets pos + heading/slope here. */
  root: THREE.Group;
  bones: Record<BoneName, THREE.Object3D>;
  boneLen: Record<BoneName, number>;
}
 ...
 */
export function buildRig(def: SpeciesDef): Rig {
  const mat = new THREE.MeshStandardMaterial({
    color: def.colors.fur,
    flatShading: true,
  });

  function makeBone(name: BoneName, len: number, up: boolean, thickness = 1): THREE.Group {
  ...
  const boneLen = {} as Record<BoneName, number>;
  ...
  for (const side of [1, -1] as const) {
    ...
    const armU = makeBone(`arm${sfx}U` as BoneName, armUpper, false);
    armU.rotation.x = Math.PI; // neutral: straight down
    armU.position.set(sideX * side, chestLen * 0.8, -def.torsoLen * 0.18);
    spine.add(armU);
    bones[`arm${sfx}U` as BoneName] = armU;
    boneLen[`arm${sfx}U` as BoneName] = armUpper;

    const armL = makeBone(`arm${sfx}L` as BoneName, armLower, false);
    armL.rotation.x = Math.PI;
    armL.position.y = -armUpper;
    armU.add(armL);
    bones[`arm${sfx}L` as BoneName] = armL;
    boneLen[`arm${sfx}L` as BoneName] = armLower;

    const legU = makeBone(`leg${sfx}U` as BoneName, legUpper, false);
    legU.rotation.x = Math.PI;
    legU.position.set(sideX * side, 0, def.torsoLen * 0.15);
    pelvis.add(legU);
    bones[`leg${sfx}U` as BoneName] = legU;
    boneLen[`leg${sfx}U` as BoneName] = legUpper;

    const legL = makeBone(`leg${sfx}L` as BoneName, legLower, false);
    legL.rotation.x = Math.PI;
    legL.position.y = -legUpper;
    legU.add(legL);
    bones[`leg${sfx}L` as BoneName] = legL;
    boneLen[`leg${sfx}L` as BoneName] = legLower;
  }
