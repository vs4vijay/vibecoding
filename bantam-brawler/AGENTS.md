# AGENTS.md — Bantam Brawler (`bantam-brawler/`)
...
- **Persistence keys are exactly `bantam.bindings.v1`** (input remapping) **and
  `bantam.muted`** (audio). No other keys, no prefixes drift.
...
`https://vs4vijay.github.io/vibecoding/bantam-brawler/`. `vite.config.ts` sets
