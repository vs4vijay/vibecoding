...
describe("mute persistence", () => {
  test("toggleMute flips, persists bantam.muted, and reports the new state", () => {
    const s = fakeStorage();
    const audio = createAudio({ storage: s });
    expect(audio.toggleMute()).toBe(true);
    expect(s.store.get("bantam.muted")).toBe("true");
    expect(audio.toggleMute()).toBe(false);
    expect(s.store.get("bantam.muted")).toBe("false");
  });

  test("muted state survives reload (restored from storage)", () => {
    const s = fakeStorage();
    s.setItem("bantam.muted", "true");
    const audio = createAudio({ storage: s });
